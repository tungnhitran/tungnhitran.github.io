...
One Docker lesson from this chapter cost me an evening, so it goes in the record: inside a compose network, the Redis URL is `redis://redis:6379` — the *service name*. Not `localhost`. Localhost inside a container is the container, staring back at you.
